...
// ---------------------------------------------------------------------------
// getProductIdFromInventoryItem(inventoryItemId) → returns the numeric Product ID
// Webhooks provide an inventory_item_id, but our waitlist uses Product IDs.
// ---------------------------------------------------------------------------
async function getProductIdFromInventoryItem(inventoryItemId) {
  const url = `https://${STORE}/admin/api/${API_VER}/graphql.json`;
  
  // We first query the InventoryItem to get the Variant, then the Variant to get the Product.
  const query = `
  ...
  try {
    ...
    const json = await res.json();
    if (json.errors || !json.data || !json.data.inventoryItem || !json.data.inventoryItem.variant) {
      return null;
    }

    // Extract the numeric ID from the Product GID (e.g. "gid://shopify/Product/123456789")
    const productGid = json.data.inventoryItem.variant.product.id;
    const parts = productGid.split("/");
    return parts[parts.length - 1];
  } catch (err) {
    console.error("Error looking up Product ID from Inventory Item:", err);
    return null;
  }
}
...
// ---------------------------------------------------------------------------
// getProductDetails(productId) → returns product title, handle, image url
// ---------------------------------------------------------------------------
async function getProductDetails(productId) {
  ...
  try {
    ...
    return {
      ...
      vendor: json.data.product.vendor,
      imageUrl: json.data.product.featuredImage ? json.data.product.featuredImage.url : null
    };
  } catch (err) {
    console.error("Error looking up Product Details:", err);
    return null;
  }
}

module.exports = { getWaitlist, updateWaitlist, getAllWaitlists, getProductIdFromInventoryItem, getProductDetails };
